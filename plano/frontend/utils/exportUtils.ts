...
export function exportActionsToCSV(rows: Action[], filename = "acoes.csv") {
  ...
  const data = rows.map(a => [
    a.id, a.title, a.start_date ?? "", a.end_date ?? "", a.status, a.department ?? "", a.pillar ?? "", a.plan_id
  ]);
...
export function exportExecutivePDF(rows: Action[], filename = "executivo.pdf") {
  ...
  const body = rows.map(a => [
    a.id, a.title, a.start_date ?? "", a.end_date ?? "", a.status, a.department ?? "", a.pillar ?? ""
  ]);

  // @ts-ignore - tipos do autotable
  doc.autoTable({
    startY: 60,
    head,
    body,
